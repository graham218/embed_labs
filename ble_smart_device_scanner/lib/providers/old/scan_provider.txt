...
class ScanNotifier extends StateNotifier<ScanState> {
  ...
  List<BleDevice> get filteredDevices {
    ...
    if (state.filterType == 'Audio Devices') {
      devices = devices.where((d) => d.device.advData.serviceUuids.contains(Guid('0000110B-0000-1000-8000-00805F9B34FB'))).toList();
    } else if (state.filterType == 'Smartwatches') {
      devices = devices.where((d) => d.name.toLowerCase().contains('watch') || d.name.toLowerCase().contains('fitbit')).toList();
    }
    return devices;
  }
}